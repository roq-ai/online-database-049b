...
import { useSearchCount, useItSupportCount, useEmailLogsCount } from 'lib/roq';

import { EmailLogsListPage as ShowCaseList } from 'pages/email-logs';

function DashboardPage() {
  const { session } = useSession();

  const searchStat = useSearchCount();

  const itSupportStat = useItSupportCount();

  const emailLogsStat = useEmailLogsCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Searches',
      result: searchStat?.data,
      isLoading: searchStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'It Supports',
      result: itSupportStat?.data,
      isLoading: itSupportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Email Logs',
      result: emailLogsStat?.data,
      isLoading: emailLogsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`email-logs`} passHref legacyBehavior>
                  <Button
