...
import { useRoqClient, useDocumentFindFirst } from 'lib/roq';
...
function DocumentEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDocumentFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DocumentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.document.update({
        data: values as RoqTypes.document,
        where: {
          id,
        },
      });
