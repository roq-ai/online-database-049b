...
import { useRoqClient, useDocumentFindManyWithCount } from 'lib/roq';
...
export function DocumentListPage(props: DocumentListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'organization', 'search.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['title.contains', 'content.contains', 'document_type.contains', 'status.contains'],
          ...params.filters,
        },
        'document',
      ),
  ...
  const { data, error, isLoading, mutate } = useDocumentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.document.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'document_type', header: 'Document Type', accessorKey: 'document_type' },
