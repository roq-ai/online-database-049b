...
import { useRoqClient, useSearchFindFirst } from 'lib/roq';
...
function SearchEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSearchFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: SearchInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.search.update({
        data: values as RoqTypes.search,
        where: {
          id,
        },
      });
      ...
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<DocumentInterface>
            ...
            fetcher={() => roqClient.document.findManyWithCount({})}
            labelField={'title'}
          />
